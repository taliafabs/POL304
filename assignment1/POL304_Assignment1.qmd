...
The code and data associated with the completion of this assignment can be accessed via [my GitHub repository](https://github.com/taliafabs/POL304H1-winter2024.git).
...
Indiscriminate violence is often viewed by academics as counterproductive because ...

This leads to the theory that a state's use of indiscriminate violence can suppress insurgent attacks by depleting insurgent groups' war resources, ability to offer civilians any perceived level of security, and by creating collective action between the state and general population against the insurgent group if civilians begin to blame the insurgency for the state's use of violence.

## Question 3

The central research hypothesis is that a state's use of indiscriminate violence can reduce insurgent attacks by depleting the resources of insurgent groups and leading civilians to believe that the insurgent group does not offer them any security, and might be responsible for the for provoking the state's violent response, which can lead to collective action between civilians and the state against insurgents.
